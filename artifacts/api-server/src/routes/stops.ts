...
router2.get("/:id/stops", requireAuth, async (req, res) => {
  try {
    ...
    const planId = one(req.params.id);

    const plan = await checkPlanAccess(planId, user.id);
    if (!plan) {
...
router2.put("/:id/stops/:stopId", requireAuth, async (req, res) => {
  try {
    const user = getUser(req);
    const planId = one(req.params.id);
    const stopId = one(req.params.stopId);
    ...
    const stop = await db.query.stopsTable.findFirst({
      where: and(eq(stopsTable.id, stopId), eq(stopsTable.planId, plan.id)),
    });
