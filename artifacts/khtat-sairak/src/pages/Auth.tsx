    ...
    <div className="min-h-screen flex items-center justify-center px-4 relative">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-md">
        <div className="glass-panel p-8 md:p-10 rounded-[2rem] relative overflow-hidden">
          ...
          <h2 className="text-3xl font-bold text-center mb-2">{isLogin ? 'أهلاً بعودتك' : 'ابدأ رحلتك معنا'}</h2>
            ...
            <AnimatePresence mode="popLayout">
              {!isLogin && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}>
                  ...
                  <Input placeholder="Ahmed" value={username} onChange={(e) => setUsername(e.target.value)} />
                </motion.div>
              )}
            </AnimatePresence>
