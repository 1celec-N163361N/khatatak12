...
import { useGetInvite, useGetStops, useVerifyPlanPassword } from '@workspace/api-client-react';
...
export default function Invite() {
  ...
  const stopsPlanId =
    inviteInfo?.planId && (!inviteInfo.hasPassword || isUnlocked) ? inviteInfo.planId : '';

  const { data: stops } = useGetStops(stopsPlanId);
  ...
  if (!inviteInfo) {
    return <div className="min-h-screen flex items-center justify-center text-xl">رابط غير صالح أو منتهي الصلاحية</div>;
  }
      ...
      <div className="min-h-screen flex items-center justify-center px-4 relative">
        ...
        <div className="glass-panel p-8 max-w-md w-full rounded-3xl relative z-10 text-center">
          ...
          <p className="text-muted-foreground mb-6">
            قام {inviteInfo.ownerName} بحماية هذه الخطة. يرجى إدخال كلمة المرور للوصول.
          </p>
