...
import { useCreateStop, useDeleteStop, useGetPlan, useSharePlan, useUpdateStop } from '@workspace/api-client-react';
import {
  ArrowRight,
  CheckSquare,
  Edit2,
...
export default function PlanView() {
  ...
  const { data: plan, isLoading, refetch } = useGetPlan(planId);
  ...
  const createStop = useCreateStop({
    mutation: {
      onSuccess: () => {
        ...
        setNewStopData({ name: '', notes: '', lat: null, lng: null });
    ...
    mutation: {
      onSuccess: (data) => {
        ...
        if (shareAction === 'whatsapp') {
          window.open(
            `https://wa.me/?text=${encodeURIComponent(`🗺️ شاركني هذه الخطة: "${plan?.title}"\n${fullUrl}`)}`,
            '_blank'
          );
          return;
        }
  ...
  const handleShare = (action: 'copy' | 'whatsapp') => {
    setShareAction(action);
    if (!planId) return;
    sharePlan.mutate({ id: planId });
  };
  ...
  const toggleActivity = async (actId: string) => {
    ...
    try {
      await patchPlan(planId, 'activities', { activities: updated });
      refetch();
    } catch {
  ...
  const saveGear = useCallback(
    async (items: GearItem[]) => {
      try {
        await patchPlan(planId, 'gear', { gearItems: items });
        refetch();
      } catch {
        toast.error("فشل الحفظ");
      }
  ...
  const toggleGearDone = (gId: string) =>
    saveGear(gearItems.map((g) => (g.id === gId ? { ...g, done: !g.done } : g)));

  const removeGearItem = (gId: string) =>
    saveGear(gearItems.filter((g) => g.id !== gId));
  ...
  const handleDragEnd = (result: DropResult) => {
    ...
    items.forEach((item, index) => {
      if (item.order !== index) {
        updateStop.mutate({ id: planId, stopId: item.id, data: { order: index } });
      }
    });
  };

  const sortedStops = [...(plan?.stops || [])]
  ...
  if (!plan) {
    return <div className="min-h-screen flex items-center justify-center text-xl">الخطة غير موجودة</div>;
  }
        ...
        <div className="p-5 border-b border-white/10 flex-shrink-0">
          ...
          <div className="flex justify-between items-start mb-2">
            ...
            <div className="flex gap-1.5 flex-shrink-0 mr-2">
              <Button size="icon" variant="ghost" onClick={() => setLocation(`/plans/${planId}/edit`)} title="تعديل">
                <Edit2 size={16} />
              </Button>
              ...
              {isAddingStop && (
                <form onSubmit={handleAddStopSubmit} className="glass-card p-4 rounded-2xl mb-5 space-y-3 border border-primary/40">
                  ...
                  <div className="flex gap-2 pt-1">
                    <Button type="submit" size="sm" className="flex-1" disabled={createStop.isPending}>
                      حفظ
                    </Button>
                    <Button
                      type="button"
                        ...
                        <Draggable key={stop.id} draggableId={stop.id} index={index}>
                          {(provided, snapshot) => (
                            <div
                              ...
                              className={`glass-card p-3.5 rounded-2xl flex items-center gap-2.5 transition-shadow ${
                            ...
                            >
                              ...
                              <div className="flex gap-1 flex-shrink-0">
                                ...
                                <Button
                                  ...
                                  onClick={() => deleteStop.mutate({ id: planId, stopId: stop.id })}
                                >
                                  <Trash2 size={14} />
                                </Button>
                              </div>
              ...
              {rawActivities.filter((a) => a.checked).length > 0 && (
                <div className="mt-5 p-4 rounded-2xl bg-primary/8 border border-primary/20">
                  <p className="text-xs text-primary font-semibold mb-2">
                    الفعاليات المختارة ({rawActivities.filter((a) => a.checked).length})
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {rawActivities
                      .filter((a) => a.checked)
                      .map((a) => (
                        <span key={a.id} className="text-sm px-2 py-1 rounded-lg bg-primary/15 text-primary">
                          {a.icon} {a.label}
                        </span>
                      ))}
                  </div>
          ...
          {tab === 'gear' && (
            <div className="p-5">
              ...
              <div className="space-y-2.5">
                ...
                {gearItems.map((item) => (
                  <div key={item.id} className={`glass-card p-3.5 rounded-2xl flex items-center gap-3 transition-all ${item.done ? 'opacity-60' : ''}`}>
