...
function MapClickHandler({
  ...
  onClick?: (lat: number, lng: number, name?: string) => void;
  interactive: boolean;
}) {
  useMapEvents({
    async click(e) {
    ...
    <div className="relative w-full h-full rounded-2xl overflow-hidden shadow-2xl border border-white/10 z-0">
      ...
      <MapContainer center={defaultCenter} zoom={12} style={{ height: '100%', width: '100%' }} zoomControl={true}>
        ...
        {polylinePoints.length > 1 && (
          <Polyline positions={polylinePoints} color="#c9a227" weight={3} opacity={0.7} dashArray="8, 6" />
        )}

        {stops
          .filter((s) => s.lat && s.lng)
          .map((stop, idx) => (
            <Marker key={stop.id} position={[stop.lat!, stop.lng!]} icon={goldIcon}>
              <Popup className="font-sans">
                <div dir="rtl" className="p-2 text-right min-w-[140px]">
        ...
        {pendingPoint && (
          <Marker position={[pendingPoint.lat, pendingPoint.lng]} icon={pendingIcon}>
            <Popup>
              <div dir="rtl" className="text-right text-sm font-bold p-1">
                نقطة جديدة — جارٍ التعرف...
              </div>
            </Popup>
          </Marker>
        )}
